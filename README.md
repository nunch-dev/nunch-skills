...
manager의 `nunch-skills-manager` 스킬이 번들 doctor를 실행해 설치된 플러그인에 필요한 항목만 찾습니다. 실제 패키지 설치는 시스템을 변경하므로, 사용할 패키지 관리자와 명령을 먼저 보여주고 승인을 받은 뒤 진행합니다. Kaneo MCP처럼 실행 파일이 아닌 연결 의존성은 자동 설치하지 않고 연결 상태와 필요한 설정을 안내합니다.
