...
type Manager struct {
	...
	store       Store
	clock       Clock
	autoRelease AutoReleaseRunner
}

type AutoReleaseRunner interface {
	Run(ctx context.Context) (AutoReleaseResult, error)
}
...
func NewWithAutoRelease(
	...
	return &Manager{
		config: config, runner: runner, store: store, clock: clock, autoRelease: autoRelease,
	}
}
...
func (manager *Manager) Run(ctx context.Context) (Result, error) {
	...
	if manager.autoRelease != nil {
		return manager.runAutoRelease(ctx, state, now)
	}
	return manager.runLegacyRefresh(ctx, state, now)
}

func (manager *Manager) runLegacyRefresh(ctx context.Context, state State, now time.Time) (Result, error) {
...
func (manager *Manager) runAutoRelease(ctx context.Context, state State, now time.Time) (Result, error) {
	result, err := manager.autoRelease.Run(ctx)
	if err != nil {
		return Result{}, manager.recordFailure(state, err)
	}
	after, err := manager.listPlugins(ctx)
	...
	if len(result.Updates) > 0 || len(dependencyReport.Missing) > 0 {
		state.PendingNotice = &PendingNotice{
			Updates: result.Updates, Dependencies: dependencyReport.Missing,
			Release: release, CompletedAt: now,
		}
	}
	if err := manager.store.Save(state); err != nil {
		return Result{}, fmt.Errorf("save verified updater success state: %w", err)
	}
	return Result{Updates: result.Updates}, nil
}
