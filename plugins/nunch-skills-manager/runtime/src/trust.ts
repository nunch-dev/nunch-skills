...
const managerPrefix = 'plugins/nunch-skills-manager/';
...
export async function verifyInstalledManagerPayload(
  pluginRoot: string,
  ...
  for (const file of files) {
    if (!file.path.startsWith(managerPrefix)) throw new TrustConflictError('manager payload path differs');
    const path = join(pluginRoot, file.path.slice(managerPrefix.length));
    const info = await lstat(path);
    if (!info.isFile() || info.isSymbolicLink()) throw new TrustConflictError('manager payload is unsafe');
    const content = await readFile(path);
    const digest = createHash('sha256').update(content).digest('hex');
    if (digest !== file.sha256) throw new TrustConflictError('manager payload digest differs');
    verified.set(file.path, content);
  }
  const hookBytes = verified.get(manifest.hook.path);
  if (hookBytes === undefined) throw new TrustConflictError('verified manager hook is missing');
  const hookFile = hookFileSchema.parse(JSON.parse(hookBytes.toString('utf8')));
  const group = hookFile.hooks.SessionStart[0];
  const hook = group?.hooks[0];
  if (group === undefined || hook === undefined) throw new TrustConflictError('manager hook is missing');
...
async function persist(path: string, original: string, updated: string): Promise<void> {
  ...
  const guard = `${path}.nunch-cas-${randomUUID()}`;
  try {
...
async function writeExclusive(path: string, content: string): Promise<void> {
  ...
  const temporary = join(parent, `.nunch-trust-${randomUUID()}.tmp`);
