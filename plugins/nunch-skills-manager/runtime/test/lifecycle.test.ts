...
test('install always includes the manager before selected leaf plugins', async () => {
  // Given
  ...
  // Then
  assert.deepEqual(backend.installed, ['nunch-skills-manager', 'git-tools', 'humanize-korean']);
});

test('update applies every installed plugin as one release', async () => {
  // Given
  ...
  // Then
  assert.deepEqual(backend.updated, ['git-tools', 'nunch-skills-manager']);
});

test('update removes old hook trust before replacing the manager payload', async () => {
  // Given
  const backend = new RecordingBackend(['nunch-skills-manager', 'git-tools']);
  const service = new LifecycleService(backend);
  ...
  assert.deepEqual(backend.events, [
    ...
    'update:git-tools',
    'remove-trust',
    'update:nunch-skills-manager',
    'ensure-trust',
  ]);
});

test('uninstall removes selected leaf plugins without removing the manager', async () => {
  // Given
  const backend = new RecordingBackend(['nunch-skills-manager', 'git-tools', 'humanize-korean']);
  const service = new LifecycleService(backend);
...
test('manager selection performs a full teardown with manager last', async () => {
  // Given
  const backend = new RecordingBackend(['nunch-skills-manager', 'git-tools']);
  const service = new LifecycleService(backend);

  // When
  await service.uninstall(['git-tools', 'nunch-skills-manager']);

  // Then
  assert.deepEqual(backend.removed, ['git-tools', 'nunch-skills-manager']);
...
test('failed install preserves plugins that existed before the operation', async () => {
  // Given
  const backend = new RecordingBackend(['nunch-skills-manager']);
...
test('manager teardown removes only the selected created plugins', async () => {
  ...
  // When
  await service.uninstall(['nunch-skills-manager']);

  // Then
  assert.deepEqual(backend.removed, ['nunch-skills-manager']);
});
