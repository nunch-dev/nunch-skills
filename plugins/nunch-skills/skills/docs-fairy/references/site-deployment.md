...
먼저 Starlight 기반 docs-site의 배포 대상을 정한 다음, 선택한 provider의 공식 방식에 따라 `config-prepared`와 `deployment-ready`를 검증합니다. 사이트 구축과 `local-complete`는 [문서 사이트](site-starlight.md)가 소유합니다.
...
| GitHub Pages | 실행 시점의 [Astro GitHub Pages 공식 가이드](https://docs.astro.build/en/guides/deploy/github/)와 GitHub 공식 문서를 확인 |
| named provider | 사용자가 지정한 provider의 공식 Astro/Starlight 배포 문서를 확인 |
...
정적 Starlight 사이트라는 이유만으로 adapter가 없다고 미리 단정하지 않습니다. 선택한 provider와 rendering 방식의 공식 절차가 요구하는 구성을 따릅니다. provider별 값을 이 reference에 영구 고정하지 않습니다.
...
- 확인일: YYYY-MM-DD
- 공식 출처: https://docs.astro.build/en/guides/deploy/github/
- Astro / Starlight / Node: {project에서 확인한 값}
- 공식 방식과 다른 점: 없음
```
...
1. [문서 사이트](site-starlight.md)의 `local-complete`가 충족됐다.
...
4. 실제 공개 URL을 확정하고 Astro `site`가 그 URL과 일치하는지 검증했다.
5. 하위 경로 배포인 경우에만 `base`를 설정하고 그 경로에서 문서 링크·asset·검색 결과 진입을 검증했다.
...
`deployment-ready`에는 실제 공개 URL이 필요합니다. Astro `site`가 공개 URL과 일치하는지 확인합니다. URL을 모르면 임의로 작성하지 말고 `config-prepared` 미충족 또는 blocker로 보고합니다.

하위 경로 배포에서만 `base`를 적용합니다. 예를 들어 GitHub Pages project site와 custom domain은 경로 조건이 다를 수 있으므로 공식 문서와 실제 URL 구조로 판단합니다. `base`를 적용했다면 production build와 실제 경로에서 다음을 확인합니다.
...
- 필요하면 Astro `site`, sitemap 생성 결과, sitemap 안의 공개 URL이 실제 배포 URL과 일치하는지 검증합니다.
- 사용자 답변 없이 자동 활성화하지 않습니다.
- sitemap 선택만으로 검색 엔진 등록, Search Console 연결이나 크롤링 요청을 수행하지 않습니다.

구현 시점의 [Astro sitemap 공식 문서](https://docs.astro.build/en/guides/integrations-guide/sitemap/)를 다시 확인합니다.

## GitHub Pages route

GitHub Pages를 선택하면 다음 순서로 준비합니다.

1. 실행 시점의 Astro와 GitHub Pages 공식 문서를 확인합니다.
2. user/organization site, project site, custom domain 중 URL 형태를 확인합니다.
3. URL에 맞춰 `site`와 조건부 `base`를 설정합니다.
...
사용자가 다른 provider를 지정하면 해당 provider의 공식 Astro 또는 정적 사이트 배포 문서를 확인합니다. 공식 방식에서 다음 값을 확인해 project 설정과 대조합니다.
...
1. 실제 원격 배포가 성공했다.
2. 공개 URL이 응답하고 `site`·조건부 `base`와 일치한다.
3. production 공개 URL에서 [문서 사이트](site-starlight.md)의 대표 흐름을 다시 검증했다.
   - 홈 CTA
   - 사이드바 이동
   - 검색 결과 진입
   - 테마 전환
