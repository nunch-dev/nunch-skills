...
7. 새 사이트 패키지가 루트 workspace에 포함되는지, 루트 `check`나 CI가 사이트 빌드를 실행하는지 확인합니다. 사용자가 저장소 전체 통합을 요청했다면 연결하고, 아니라면 임의로 CI를 바꾸지 말고 현재 참여 여부를 완료 보고에 남깁니다.
...
- 완료 전에 문서의 **정본(SSOT)** 과 동기화 방식을 확정합니다. 기본 제안은 사이트 문서를 정본으로 두고 GitHub에서 바로 읽을 가치가 있는 원본은 요약 + 사이트 링크 stub으로 남기는 방식입니다. 원본을 정본으로 유지한다면 결정적 생성·동기화 절차를 둡니다. 중복 유지를 선택하면 동기화 책임과 갱신 절차를 문서화합니다. 관리되지 않는 두 사본을 남긴 채 완료하지 않으며, edit link는 정본을 가리켜야 합니다.
