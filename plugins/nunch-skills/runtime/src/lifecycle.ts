import { installerName } from './installer-identity.ts';
...
export class LifecycleService {
  ...
  constructor(
    ...
    options: { includeInstaller?: boolean } = {},
  ) {
    this.backend = backend;
    this.progress = progress;
    this.includeInstaller = options.includeInstaller ?? true;
  }

  async install(selected: string[]): Promise<void> {
    const leafPlugins = unique(selected.filter((name) => name !== installerName)).sort();
    const targets = this.includeInstaller ? [installerName, ...leafPlugins] : leafPlugins;
    ...
    if (this.includeInstaller) {
      await this.stage('install', 'trust', installerName, () => this.backend.ensureTrust());
    }
  }
  ...
  async update(): Promise<void> {
    ...
    if (this.includeInstaller) {
      await this.stage('update', 'trust', installerName, () => this.backend.removeTrust());
      if (installed.includes(installerName)) {
        await this.stage('update', 'plugins', installerName, () => this.backend.updatePlugin(installerName));
      } else {
        await this.stage('update', 'plugins', installerName, () => this.backend.installPlugin(installerName));
      }
      const legacyPlugins = installed.filter((name) => name !== installerName).sort();
      for (const target of legacyPlugins) {
        await this.stage('update', 'plugins', target, () => this.backend.removePlugin(target));
      }
      await this.stage('update', 'trust', installerName, () => this.backend.ensureTrust());
      return;
    }
    const leaves = installed.sort();
  ...
  async uninstall(
    ...
    options: UninstallOptions = {
      removeTrust: selected.includes(installerName),
      removeMarketplace: selected.includes(installerName),
    },
  ): Promise<void> {
    const installed = await this.backend.listInstalled();
    if (selected.includes(installerName)) {
      const installedSet = new Set(installed);
      const leaves = unique(selected)
        .filter((name) => name !== installerName && installedSet.has(name))
        .sort();
      for (const target of [...leaves, installerName]) {
        await this.stage('uninstall', 'plugins', target, () => this.backend.removePlugin(target));
      }
      if (options.removeTrust) await this.stage('uninstall', 'trust', installerName, () => this.backend.removeTrust());
