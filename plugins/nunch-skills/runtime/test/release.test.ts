...
test('accepts standard npm metadata while verifying protected files', async () => {
  ...
  const runtime = 'plugins/nunch-skills/runtime/nch-installer.mjs';
  ...
  const manifest = {
    ...
    plugin: { path: 'plugins/nunch-skills/.codex-plugin/plugin.json', sha256: 'd'.repeat(64) },
    hook: { path: 'plugins/nunch-skills/hooks/hooks.json', sha256: 'e'.repeat(64) },
    scripts: [],
...
test('authenticates the npm manifest against the complete Git release', async () => {
  ...
  const pluginManifest = '{"name":"nunch-skills","version":"1.2.3"}\n';
  await write(repository, 'plugins/nunch-skills/.codex-plugin/plugin.json', pluginManifest);
  await write(staging, 'plugins/nunch-skills/.codex-plugin/plugin.json', pluginManifest);
...
test('rejects symlinks in an npm update archive', async () => {
  ...
  const manifest: ReleaseManifest = {
    ...
    plugin: { path: 'plugins/nunch-skills/.codex-plugin/plugin.json', sha256: 'e'.repeat(64) },
    hook: { path: 'plugins/nunch-skills/hooks/hooks.json', sha256: 'f'.repeat(64) },
    scripts: [],
    runtime: {
      path: 'plugins/nunch-skills/runtime/nch-installer.mjs',
      sha256: '0'.repeat(64),
    },
  };
