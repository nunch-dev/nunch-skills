...
Place `dependencies.json` at the plugin root. The installer reads it only for installed plugins.
...
- `executables` declares commands the installer can probe directly without a shell.
...
- `manual` lists connections or setup steps that the installer must not install automatically.
