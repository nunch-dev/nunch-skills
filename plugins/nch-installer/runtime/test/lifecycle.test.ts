...
test('install always includes the installer before selected leaf plugins', async () => {
  // Given
  ...
  // Then
  assert.deepEqual(backend.installed, ['nch-installer', 'git-tools', 'humanize-korean']);
});

test('update applies every installed plugin as one release', async () => {
  // Given
  const backend = new RecordingBackend(['nch-installer', 'git-tools']);
  const service = new LifecycleService(backend);
  ...
  // Then
  assert.deepEqual(backend.updated, ['git-tools', 'nch-installer']);
});
...
test('installer selection performs a full teardown with installer last', async () => {
  ...
  // When
  await service.uninstall(['git-tools', 'nch-installer']);

  // Then
  assert.deepEqual(backend.removed, ['git-tools', 'nch-installer']);
...
test('failed install preserves plugins that existed before the operation', async () => {
  // Given
  const backend = new RecordingBackend(['nch-installer']);
...
test('installer teardown removes only the selected created plugins', async () => {
  ...
  // When
  await service.uninstall(['nch-installer']);

  // Then
  assert.deepEqual(backend.removed, ['nch-installer']);
});
