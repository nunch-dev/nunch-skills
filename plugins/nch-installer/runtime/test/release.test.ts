...
test('accepts standard npm metadata while verifying protected files', async () => {
  ...
  const runtime = 'plugins/nch-installer/runtime/nch-installer.mjs';
  ...
  const manifest = {
    ...
    plugin: { path: 'plugins/nch-installer/.codex-plugin/plugin.json', sha256: 'd'.repeat(64) },
    hook: { path: 'plugins/nch-installer/hooks/hooks.json', sha256: 'e'.repeat(64) },
    scripts: [],
    plugins: [{ name: 'nch-installer', version: '1.2.3' }],
  };
...
test('authenticates the npm manifest against the complete Git release', async () => {
  ...
  const marketplace = `${JSON.stringify({
    name: 'nunch-skills',
    plugins: [{ name: 'nch-installer', source: { source: 'local', path: './plugins/nch-installer' } }],
  })}\n`;
  ...
  const pluginManifest = '{"name":"nch-installer","version":"1.2.3"}\n';
  await write(repository, 'plugins/nch-installer/.codex-plugin/plugin.json', pluginManifest);
  await write(staging, 'plugins/nch-installer/.codex-plugin/plugin.json', pluginManifest);
...
test('rejects symlinks in an npm update archive', async () => {
  ...
  const manifest: ReleaseManifest = {
    ...
    plugin: { path: 'plugins/nch-installer/.codex-plugin/plugin.json', sha256: 'e'.repeat(64) },
    hook: { path: 'plugins/nch-installer/hooks/hooks.json', sha256: 'f'.repeat(64) },
    scripts: [],
    runtime: {
      path: 'plugins/nch-installer/runtime/nch-installer.mjs',
      sha256: '0'.repeat(64),
    },
  };
