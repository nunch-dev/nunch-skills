...
import { hookTrustHash, TrustEditor, verifyInstallerPayload } from '../src/trust.ts';

const trustId = 'nch-installer@nunch-skills:hooks/hooks.json:session_start:0:0';
...
test('hashes the canonical platform hook identity', () => {
  ...
  const identity = {
    ...
    hooks: [
      {
        async: false,
        command: 'node installer.mjs hook',
...
test('rejects trust when the installed installer payload is tampered', async () => {
  // Given
  const root = await mkdtemp(join(tmpdir(), 'installer-payload-'));
      ...
      SessionStart: [
        {
          ...
          hooks: [
            {
              type: 'command',
              command: `node "\${PLUGIN_ROOT}/runtime/nch-installer.mjs" hook session-start`,
              commandWindows: 'powershell installer.ps1',
              timeout: 15,
  ...
  const contents = new Map([
    ...
    ['runtime/nch-installer.mjs', 'runtime\n'],
  ...
  const releaseFile = (path: string, relative: string) => ({
    path: `plugins/nch-installer/${path}`,
  ...
  const manifest: ReleaseManifest = {
    ...
    runtime: releaseFile('runtime/nch-installer.mjs', 'runtime/nch-installer.mjs'),
  };
  await writeFile(join(root, 'runtime/nch-installer.mjs'), 'tampered\n');

  // When / Then
  await assert.rejects(() => verifyInstallerPayload(root, manifest, 'darwin'), /digest/);
});
