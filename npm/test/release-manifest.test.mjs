...
async function releaseFixture(context, { largeBinary = false } = {}) {
  ...
  for (const [index, path] of packageFiles.entries()) {
    const content = largeBinary && index === 2 ? Buffer.alloc(2 * 1024 * 1024, 0x5a) : `fixture ${path}\n`;
    await writeFixtureFile(repository, path, content);
    await writeFixtureFile(staging, path, content);
  }
  await writeFixtureFile(repository, ".agents/plugins/marketplace.json", `${JSON.stringify({
    name: "nunch-skills",
    plugins: [
  ...
  await writeFixtureFile(repository, "plugins/git-tools/.codex-plugin/plugin.json", '{"name":"git-tools","version":"0.2.1"}\n');
  await writeFixtureFile(repository, "plugins/nunch-skills-manager/.codex-plugin/plugin.json", '{"name":"nunch-skills-manager","version":"1.2.3"}\n');
  await writeFixtureFile(repository, "plugins/nunch-skills-manager/hooks/hooks.json", "hook\n");
  await writeFixtureFile(repository, "plugins/nunch-skills-manager/scripts/run-manager.ps1", "powershell\n");
  await writeFixtureFile(repository, "plugins/nunch-skills-manager/scripts/run-manager.sh", "shell\n");
...
test("release manifest reads multi-megabyte binaries from the pinned commit", async (context) => {
  // Given
  const fixture = await releaseFixture(context, { largeBinary: true });

  // When
  const result = await generateReleaseManifest({ ...fixture, tag: "v1.2.3", dryRun: true });

  // Then
  assert.equal(result.manifest.binaries.length, 6);
});
...
test("release manifest is deterministic and generated outside the pinned Git tree", async (context) => {
  ...
  assert.equal(first.manifest.binaries.length, 6);
...
test("release manifest rejects staging content that differs from the pinned commit", async (context) => {
  ...
  await writeFile(join(fixture.staging, binaryPath), "tampered\n");
