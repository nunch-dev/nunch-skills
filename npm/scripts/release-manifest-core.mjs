...
const SCRIPT_PATHS = [
  "plugins/nunch-skills-manager/scripts/run-manager.ps1",
  "plugins/nunch-skills-manager/scripts/run-manager.sh"
];
const BINARIES = [
...
function declaredPackageFiles(packageManifest) {
  ...
  if (!Array.isArray(packageManifest.files) || packageManifest.files.some((value) => typeof value !== "string" || !isSafeRelativePath(value))) {
...
function pluginCatalog(gitFiles) {
  ...
  const plugins = marketplace.plugins.map((entry) => {
    ...
    if (manifest.name !== entry.name || typeof manifest.version !== "string" || !SEMVER_PATTERN.test(manifest.version)) {
...
async function verifyStageMatchesGit(repo, commit, staging, paths, gitFiles) {
...
async function generateReleaseManifest(input) {
  ...
  const binaryPaths = new Set(BINARIES.map(([, path]) => path));
  for (const binaryPath of binaryPaths) {
    if (!packageFiles.includes(binaryPath)) {
      throw new ReleaseManifestError(`package files is missing platform binary: ${binaryPath}`);
    }
  }
  await verifyStageMatchesGit(repo, input.commit, staging, packageFiles, gitFiles);
  const npmFiles = packageFiles
    .filter((path) => !binaryPaths.has(path))
    .map((path) => releaseFile(path, gitFiles.get(path)));
  const binaries = BINARIES.map(([platform, path]) => ({
    platform,
    gitPath: path,
    npmPath: path,
    sha256: sha256(gitFiles.get(path))
  }));
  const protectedFiles = [MARKETPLACE_PATH, PLUGIN_PATH, HOOK_PATH, ...SCRIPT_PATHS];
